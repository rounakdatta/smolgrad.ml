...
let test_custom_operator () =
  ...
  let abba = a + b in
  Alcotest.(check (float 0.0))
    "Variable: adds up correctly with custom operator"
    6.0
    (Variable.data abba);
;;
...
let test_graph_construction () =
  ...
  Alcotest.(check (list (float 0.0)))
    "Variable: Dependency graph is constructed correctly for c"
    (List.map (fun x -> Variable.data x) [Variable.(a * b); Variable.(b ** 3.0)])
    (List.map (fun x -> Variable.data x) (Variable.dependencies c));

  Alcotest.(check (list (float 0.0)))
    "Variable: Dependency graph is constructed correctly for d"
...
let test_backpropagation () =
  ...
  Alcotest.(check (float 0.0))
    "Variable: Backpropagation yields correct gradient for a for a complex graph"
    2.0
    (Variable.grad a);

  Alcotest.(check (float 0.0))
    "Variable: Backpropagation yields correct gradient for b a complex graph"
    8.0
    (Variable.grad b);
;;
